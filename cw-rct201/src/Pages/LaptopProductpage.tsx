...
const LaptopProductPage = () => {
  ...
  const searchData = async (searchText: string) => {
    ...
    let response = await axios.get(
      `http://localhost:4040/products?category=${searchText}`
    );
  ...
  return (
    ...
    <Text textAlign={"center"} fontSize="40px" marginTop={"30px"} fontWeight={"bold"}>Laptops</Text>
    <Flex justifyContent={"center"} gap="30px" marginTop={"10px"} >
    
        <Image width={"75px"} height="75px" borderRadius={"50%"} border="1px solid black"
        ...
        onClick={()=>setSearchText("lenovo")} alt="" />

        <Image width={"75px"} height="75px" borderRadius={"50%"} border="1px solid black"
         onClick={()=>setSearchText("hp")}
         src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/ad/HP_logo_2012.svg/1200px-HP_logo_2012.svg.png" alt="" />
        <Image width={"75px"} height="75px" borderRadius={"50%"} border="1px solid black" 
         onClick={()=>setSearchText("Dell")}
        src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Dell_logo_2016.svg/1200px-Dell_logo_2016.svg.png" alt="" />
    </Flex>
    <Center w="100vw">
